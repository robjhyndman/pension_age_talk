...
\placefig{1.5}{1.5}{width=13cm, trim=0 20 0 0, clip=true}{ageing}

\vspace*{3cm}\pause

  1. advances in medical care leading to longer life
  2. high fertility rates after World War II
  3. large decline in fertility rates over past thirty years.

\pause\vspace*{0.1cm}

\begin{block}{Consequently:}
\begin{itemize}\tightlist
\item older people leaving workforce, fewer workers replacing them
\item increasing old-age dependency ratio (OADR)
\end{itemize}
\end{block}
...
\begin{alertblock}{}
\begin{equation*}
  \text{OADR} = \frac{\text{number of people aged over pension age}}{\text{number of people aged 15 to pension age}}
\end{equation*}
\end{alertblock}
...
## Old Age Dependency Ratio in Australia

```{r oadrhistory}
loadd(OADR_history)
loadd(history_pop)
loadd(pension_age)
OADR_65 <- oadr(history_pop, filter(pension_age, Policy == "Age65"))
OADR_65 %>% 
    ggplot(aes(x = Year, y = OADR*100)) +
    geom_line(lwd=1, col=pension_colors["Age65"]) +
    geom_line(col = "black", size = .75, data = OADR_history) +
    labs(y = "Old Age Dependency Ratio (%)") +
    annotate("text", x=2010, y=24, label="Pension age 65", col=pension_colors["Age65"])
```
...
## Functional linear model for fertility
...
\end{block}
  ...
  * $\{\beta_{1,j},\dots,\beta_{T,j}\}$ is automatically modelled with ARIMA process
  * $\widehat{\beta}_{T+h\mid T,j}$ is an $h$-step forecast from the ARIMA model

\begin{alertblock}{}
\centerline{$\displaystyle\widehat{y}_{T+h\mid T}(x)= \widehat{\mu}(x)+\sum_{j=1}^{J}\phi_j(x)\widehat{\beta}_{T+h\mid T,j}$}
\end{alertblock}
...
\end{block}
  ...
  * $y_t(x)= \log(f_t(x))$ 
  * $s_t(x)=$ smoothed version of $y_t(x)$
  * $\varepsilon_{t,x} \sim$ iid N(0,1). $e_t(x)$ serially uncorrelated with mean zero
  * $\mu(x)=$ mean $s_t(x)$ across years. 
  * $\phi_j(x)$ and $\beta_{t,j}$ are principal components & scores of $s_t(x)-\mu(x)$. 
  * $\{\beta_{1,j},\dots,\beta_{T,j}\}$ is modelled with ARIMA \rlap{processes}

## Functional linear models for mortality
...
\end{block}
  ...
  * $y_t(x)= \log(m_t^M(x)m_t^F(x))$ and $\log(m_t^M(x)/m_t^F(x))$ 
  * $s_t(x)=$ smoothed version of $y_t(x)$
  * $\varepsilon_{t,x} \sim$ iid N(0,1). $e_t(x)$ serially uncorrelated with mean zero
  * $\mu(x)=$ mean $s_t(x)$ across years. 
  * $\phi_j(x)$ and $\beta_{t,j}$ are principal components & scores of $s_t(x)-\mu(x)$. 
  * $\{\beta_{1,j},\dots,\beta_{T,j}\}$ is modelled with ARIMA or ARFIMA \rlap{processes}


## Functional linear models for migration
...
\end{block}
  ...
  * $y_t(x)= (G_t^M(x) + G_t^F(x))$ and $(G_t^M(x) - G_t^F(x))$
  * $s_t(x)=$ smoothed version of $y_t(x)$
  * $\varepsilon_{t,x} \sim$ iid N(0,1). $e_t(x)$ serially uncorrelated with mean zero
  * $\mu(x)=$ mean $s_t(x)$ across years. 
  * $\phi_j(x)$ and $\beta_{t,j}$ are principal components & scores of $s_t(x)-\mu(x)$. 
...
## Population forecasts
  ...
  * Fit similar models to fertility rates, mortality rates, migration numbers
...
\begin{block}{For each year of each simulated population}
\begin{itemize}\tightlist
  \item Draw from forecast distribution of fertility rates: $f_t(x)$
  \item Generate number of births from Poisson population: $B_t$
  \item Draw from forecast distribution of mortality rates: $m_t^F(x)$, $m_t^M(x)$
  \item Generate number of deaths by age and sex: $D_t(x) \sim \text{Poisson}(P_t(x)m_t(x))$
  \item Draw from forecast distribution of migration: $G_t^F(x)$, $G_t^M(x)$
  \item Compute population for next year from
    $$P_{t+1}(x) = P_t(x) + B_t1(x=0) - D_t(x) + G_t(x)$$
\end{itemize}
\end{block}

Repeat for all future years. Repeat for multiple simulated \rlap{populations.}

\begin{textblock}{5}(11,0.5)
\fontsize{12}{13}\sf\textcolor{white}{Slightly over-simplified}
\end{textblock}

## Population forecasts

```{r aussim, fig.show='animate', interval=1, message=FALSE, aniopts='controls=none,autoplay,loop,width=14.5cm'}
loadd(aussim_df)
aussim_df %>%
  filter(Rep %in% seq(1,2000,by=100)) %>%
  ggplot(aes(x=Age, y=Population, group=Year, col=Year)) +
  geom_line() +
  facet_grid(. ~ Sex) +
  scale_color_gradientn(colours = rainbow(6)) +
  transition_states(Rep, transition_length=0, state_length=1)
```
...
loadd(pyramid_plot)
pyramid_plot +
  plot_annotation(title = "Forecast population for 2038 compared to 2019")
```
...
loadd(oadr_plots)
oadr_plots +
  geom_hline(aes(yintercept=oadr_target*100), color="#0072B2") +
...
\placefig{4}{3.2}{width=11cm, height=10cm}{lifestages}
...
\placefig{0.5}{2.8}{width=3cm}{forecast.png}
\placefig{2}{5.4}{width=3cm}{demography.png}
